...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-6 gap-6">
  ...
  <div class="d-flex flex-column justify-content-center">
    <div class="d-flex align-items-center mb-1">
      <h5 class="mb-0">Edit Bon Barang Kode {{ $tr->transaksi_code ?? '' }}</h5>
      <span class="badge bg-label-primary me-2 ms-2 rounded-pill">Update</span>
      <input type="hidden" name="transaksi_id" id="transaksi_id" value="{{ $tr->transaksi_id }}">
    </div>
        ...
        <div class="col-6 pt-3 pb-3 mb-3">
          <div class="form-floating form-floating-outline">
            <select id="seksi_id" name="seksi_id" class="select2 form-select form-select-lg"
              ...
              <option value="{{ $tr->Seksi->seksi_id }}">{{ $tr->Seksi->seksi_kode ?? '' }} - {{ $tr->Seksi->seksi_name
                }}</option>
          ...
          <table class="datatables-ajax table" id="tableConfirm">
            ...
            <tbody id="bodyConfirm">
              ...
              <tr role="row" class="odd ">
                ...
                <td class="product-name">{{ $item->Barang->barang_name }}</td>
                <td>{{ $item->qty }}</td>
                <td>{{ $item->Barang->satuan }}</td>
                <td>
                  <button class="btn btn-xs rounded-pill btn-danger removeButton">Hapus</button>
                </td>
              </tr>
              @endforeach
            </tbody>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function tambahBarang() {
        ...
        let namaExists = false;
        $('#tableConfirm tbody tr').each(function(index) {
            const rowNama = $(this).find('td').eq(1).text().trim(); // Get the text from the third cell in the row
            if (rowNama === nama) {
                namaExists = true;
        ...
        if (namaExists) {
            Swal.fire({
                ...
                , text: `Barang dengan nama ${nama} sudah ada di keranjang`
            , });
            return;
        }
    ...
    $(document).ready(function() {
        ...
        // $('#bodyConfirm tr').each(function() {
        //     var confirmQty = parseInt($(this).find('td:eq(2)').text().trim(), 10);
        //     var barangCode = $(this).find('td:eq(0)').text().trim();

        //     // Find the corresponding row in the list table
        //     $('tbody.list tr').each(function() {
        //         var listBarangCode = $(this).find('small').text().trim().split(': ')[1];
        //         var listQty = parseInt($(this).find('.qtyReal').text().trim(), 10);
        //         if (barangCode === listBarangCode) {
        //             var newQty = listQty - confirmQty;
        //             $(this).find('.qtyReal').text(newQty);
        //         }
        //     });
        // });
